...
<template>
  <div class="tablepage">
    <div class="addGoods" v-if="mode == 'base'">
      <el-button @click="addPinGoods()" type="primary">{{res}}</el-button>
    </div>
...
<script>
import {goodsClassAll,goodsOneClassEnable,goodsList,getGoodsUpper,shopCustomData,pinTuanList} from "@/api/request/goods";
...
export default {
  ...
  watch: {
    containerData: function() {
      this.$emit("listentPingoodData", this.containerData);
    },
    options: function() {
