        ...
        <el-tabs class="tabs" value="name1">
          <el-tab-pane label="商城页面" name="name1">
            <div class="panel">
              ...
              <div class="panel-content">
                <el-radio-group type="button" v-model="urldata" size="large" @on-change="btnfunc">
                  ...
                  <el-radio label="/pages/category/category">分类导航</el-radio>
            ...
            <div class="panel">
              ...
              <div class="panel-content">
                <el-radio-group type="button" v-model="urldata" size="large" @on-change="btnfunc">
                   <el-radio label="/pages/user/user">个人中心</el-radio>
                  <el-radio label="/pages/order/index">全部订单</el-radio>
                  <el-radio label="/pages/order/index?status=12">待付款订单</el-radio>
                  <el-radio label="/pages/order/index?status=2">待发货订单</el-radio>
                  <el-radio label="/pages/order/index?status=3">待收货订单</el-radio>
                  <el-radio label="/pages/order/index?status=5">已完成订单</el-radio>
                   <el-radio label="/pages/user/collect">我的收藏</el-radio>
                  <el-radio label="/pages/user/coupon">优惠券</el-radio>
                  <el-radio label="/pages/address/address">收货地址管理</el-radio>
                </el-radio-group>
              </div>
            </div>
            <div class="panel">
  ...
  methods: {
    ...
    getData(page) {
      ...
      getGoodsUpper({
        size: this.size,
        current: page ? page : 0,
        keyword: this.idAndName ? this.idAndName : ''
      }).then(res => {
        if (res.code == 200) {
          let data = res.data
          ...
          data.records.map((v, i) => {
            goods.push({
              ...v,
              link:'../../pagesA/product/product?id='+v.id,
              isSelected: false
            })
          });
          goods.map((v, i) => {
