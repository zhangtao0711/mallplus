<template>
    <div :id="id" class="orderArea"></div>
</template>

<script>
	import echarts from 'echarts'
    import echartsTheme from "@/components/echarts/theme/westeros.json";
import {orderStatusStatics, orderStatic, userStatic,goodsSort,goodsCollect} from '@/api/home'
    export default {
        data(){
            return {
				id:"lineChart",
				datetime:[],
				 series_data:[],
                 legend_data:[],
                myChart:null,
            }
        },
        created() {
                 let text,legend_data=[],series_data=[];
                  orderStatusStatics({}).then(res => {
                                                         if (res.code == 200) {
                                                             let map = res.data;
                                                            for (var key in map) {
                                                             legend_data.push(key);
                                                             let data=[];
                                                              console.log(map[key]);
                                                             if(map[key] && map[key]!=null){
data.push(map[key]['a1']);data.push(map[key]['a2']);data.push(map[key]['a3']);data.push(map[key]['a4']);data.push(map[key]['a5']);data.push(map[key]['a6']);
data.push(map[key]['a7']);data.push(map[key]['a8']);data.push(map[key]['a9']);data.push(map[key]['a10']);data.push(map[key]['a11']);data.push(map[key]['a12']);

                                                             }


                                                             console.log(data)
                                                             series_data.push({name: key,areaStyle: {normal: {}}, type:'line',stack: '总量',data: data});

                                                             }

                                                               this.legend_data=legend_data;
                                                                 this.series_data=series_data;
                                                                this.loadChart();
                                                         }

                                                     });


                       },
		beforeDestroy() {
			if (!this.myChart) {
				return
			}
			this.myChart.dispose();
			this.myChart = null;
        },
        methods: {
			loadChart(){
  				this.$nextTick(() => {
                    echarts.registerTheme('westeros', echartsTheme)
                    this.myChart = echarts.init(document.getElementById(this.id),'westeros');
                    this.myChart.setOption(this.initOption());
                })
			},
			initOption(){
				let data = {
					title: {
						text: ''
					},
					tooltip : {
						trigger: 'axis',
						axisPointer: {
							type: 'cross',
							label: {
								backgroundColor: '#6a7985'
							}
						}
					},
					legend: {
						data:this.legend_data
					},
					grid: {
						left: '3%',
						right: '4%',
						bottom: '3%',
						containLabel: true
					},
					xAxis : [
						{
							type : 'category',
							boundaryGap : false,
							data : ['一月','二月','三月','四月','五月','六月','七月','八月','九月','十月','十一月','十二月']
						}
					],
					yAxis : [
						{
							type : 'value'
						}
					],
					series :this.series_data,
					series1 : [
						{
							name:'股票',
							type:'line',
							stack: '总量',
							areaStyle: {normal: {}},
							data:[120, 132, 101, 134, 90, 230, 210]
						},
						{
							name:'基金',
							type:'line',
							stack: '总量',
							areaStyle: {normal: {}},
							data:[220, 182, 191, 234, 290, 330, 310]
						},
						{
							name:'债券',
							type:'line',
							stack: '总量',
							areaStyle: {normal: {}},
							data:[150, 232, 201, 154, 190, 330, 410]
						},
						{
							name:'期货',
							type:'line',
							stack: '总量',
							areaStyle: {normal: {}},
							data:[320, 332, 301, 334, 390, 330, 320]
						},
						{
							name:'储蓄',
							type:'line',
							stack: '总量',
							label: {
								normal: {
									show: true,
									position: 'top'
								}
							},
							areaStyle: {normal: {}},
							data:[820, 932, 901, 934, 1290, 1330, 1320]
						}
					]
				}
				return data;
			},
		},
        watch: {
            // id:()=>{
            //     this.initOption()
            // }
        }
    }
</script>

<style lang="less">

</style>
