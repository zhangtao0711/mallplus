            ...
            <draggable v-model="currentTemplate" v-bind="dragOptions">
              <transition-group type="transition">
                ...
                >
                  ...
                  <iconBannerTem v-if="item.components == 'iconBanner'" :options="item.options" />
                </div>
              </transition-group>
            </draggable>
          </div>
...
import iconBannerTem from "./components/iconBannerTem/iconBannerTem";
// import BottomenuConfig from "./componentConfig/bottomenuConfig/bottomenuConfig";
  ...
  methods: {
    ...
    validateData() {
      ...
      for (let j = 0; j < _this.currentTemplate.length; j++) {
        ...
        if (
          v.components == "mofang" &&
          v.options.hasOwnProperty("combineLists")
        ) {
    ...
    back() {
      if (this.tabstatus) {
        this.$router.push(
          "/@/views/shop/smallprogram/PageAdmin?tabstatus=" + this.tabstatus
        );
      } else {
        this.$router.go(-1);
      }
