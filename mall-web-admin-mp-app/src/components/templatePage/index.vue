		...
		<div class="decorate-content">
			...
			<div class="decorate-content-left">
				<div class="left-item" v-for="(item, index) in collists" :key="index">
					...
					>
						<div class="listitem pointer" v-for="(childItem, childIndex) in item.toolsChildren" :key="childIndex" @click="addGoodsItem(childItem)">
							...
							<transition-group type="transition">
								...
								>
									...
									<div :class="[item.click ? (showConfrim ? 'confirmDelShow' : 'confirmDelHide') : 'confirmDelHide']" style="top: 10px; right: 10px;">
										...
										<div class="btn-group">
											<el-button type="primary" ghost size="small" @click.stop="confrimDelete(item, index)">确定</el-button>
			...
			<div class="decorate-content-right">
				...
				<BannerConfig
					...
					:editable="editable"
				/>
				<ShopInfoConfig v-if="currentClick.components == 'shangpinxiangqing'" v-on:listenToForm="changeTemplate" :options="currentClick" :editable="editable" />
				<ImgConfig
	...
	methods: {
		...
		clickItem(item, index) {
			...
			_this.currentTemplate.map((v, i) => {
				if (i == index) {
					let selects = [];
					if (v.components == 'danlieshangpin' || v.components == 'shuanglieshangpin') {
						if (v.options.hasOwnProperty('colorGroup') && v.options.colorGroup.length > 0) {
