<template>
  <div class="detail-container">
    <el-card shadow="never">
      <span class="font-title-medium">{{orderReturnApply.typeStr}}</span>
      <el-table
        border
    ...
    <el-card shadow="never" class="standard-margin">
      ...
      <div class="form-container-border">
        ...
        <el-row v-if="orderReturnApply.type==1 ||orderReturnApply.type==3">
          ...
          申请退款 ￥{{orderReturnApply.returnAmount}};  ￥
          ...
          <el-col class="form-border font-small" style="height:52px" :span="18">
            <el-select size="small"
              ...
              <el-option v-for="address in companyAddressList"
                         :key="address.id"
                         :value="address.id"
                         :label="address.province+'-'+address.city+'-'+address.region+'-'+address.detailAddress">
              </el-option>
            </el-select>
          </el-col>
        </el-row>
        <el-row v-if="orderReturnApply.companyAddressId">
      ...
      <div style="margin-top:15px;text-align: center" v-show="orderReturnApply.status===0">
        <el-button type="primary" size="small" @click="handleUpdateStatus(1)">确认{{orderReturnApply.typeStr}}</el-button>
        <el-button type="danger" size="small" @click="handleUpdateStatus(3)">拒绝{{orderReturnApply.typeStr}}</el-button>
      </div>
      <div style="margin-top:15px;text-align: center" v-show="orderReturnApply.status===1 && (orderReturnApply.type==2 ||orderReturnApply.type==3)">
        <el-button type="primary" size="small" @click="handleUpdateStatus(2)">确认{{orderReturnApply.typeStr}}</el-button>
      </div>
    </el-card>
  </div>
</template>
<script>
  ...
  import {fetchList} from '@/api/ums/memberAddress';
  ...
  export default {
    ...
    computed: {
      ...
      currentAddress() {
        ...
        if(!id){
          id=this.updateStatusParam.companyAddressId;
        }
    ...
    methods: {
      ...
      getDetail() {
        getApplyDetail(this.id).then(response => {
          ...
          this.listQuery.memberId=this.orderReturnApply.returnName;
          //退货中和完成
      ...
      getCompanyAddressList() {
        fetchList(this.listQuery).then(response => {
          ...
          this.companyAddressList = response.data.records;

        });
      },
      handleUpdateStatus(status){
      this.updateStatusParam.orderId=this.orderReturnApply.orderId;
