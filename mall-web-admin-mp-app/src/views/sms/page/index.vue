<template>
  <div class="app-container">
    ...
    <el-card class="operate-container" shadow="never">
      ...
      <span>数据列表</span>
      <el-button class="btn-add" @click="addSmsDiyPage()" size="mini">添加</el-button>
    </el-card>
    <div class="table-container">
      ...
      >
        ...
        <el-table-column label="操作" width="300" align="center">
          <template slot-scope="scope">
            <el-button size="mini" @click="preview(scope.$index1, scope.row)">预览</el-button>
            <el-button size="mini" @click="copyFun(scope.$index, scope.row)">复制</el-button>
...
export default {
  ...
  methods: {
    ...
    preview(index1, row) {
      this.$router.push(
        `/@/components/templatePage/index1?type=` +
          row.type +
          "&id=" +
          row.id +
          "&status=" +
    ...
    copyFun(index, row) {
      this.$router.push(
        `/@/components/templatePage?role=1&type=` + row.type + "&id=" + row.id
      );
    },
    handleDelete(index, row) {
      this.$confirm("是否要删除该类型", "提示", {
        ...
        deleteSmsDiyPage(row.id).then(response => {
          this.$message({
            message: "删除成功",
            type: "success",
            duration: 1000
