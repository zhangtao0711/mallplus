	...
	<view class="container">
		...
		<view class="carousel-section">
			...
			<swiper class="carousel" circular @change="swiperChange">
				<swiper-item v-for="(item, index) in carouselList" :key="index" class="carousel-item" @click="navToDetailPageL(item.url)"><image :src="item.pic" /></swiper-item>
			</swiper>
			<!-- 自定义swiper指示器 -->
		...
		<!-- 分类 -->
		<view class="cate-section" v-if="cat_list && cat_list.length > 0">
			<view class="cate-item" @click="navToTabPageList(item1)" v-for="(item1, index) in cat_list" :key="index">
				<image :src="item1.pic"></image>
				<text>{{ item1.name }}</text>
			</view>
		</view>
		<view class="cate-section" v-if="cat_list1 && cat_list1.length > 0">
			<view class="cate-item" @click="navToTabPageList(item1)" v-for="(item1, index) in cat_list1" :key="index">
				<image :src="item1.pic"></image>
				<text>{{ item1.name }}</text>
			</view>
		</view>
...
export default {
	...
	methods: {
		...
		async getNewProductList(type = 'add', loading) {
			...
			let params = { pageNum: this.pageNum, storeId: this.id };
		...
		async loadData2() {
			let params = { id: this.id };
			let data = await Api.apiCall('get', Api.member.store, params);
			...
			if (data) {
				...
				let cat_list = data.categoryList;
				this.cat_list = cat_list || [];
				this.cat_list1 = data.categoryList1;
		...
		navToTabPageList(item) {
			...
			let id = item.id;
			uni.navigateTo({
				url: `../../pagesA/product/list?productAttributeCategoryId=${id}`
			});
		},
		//详情页
