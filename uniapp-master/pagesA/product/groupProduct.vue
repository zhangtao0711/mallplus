	...
	<view class="container">
		...
		<view class="introduce-section">
			...
			<view class="bot-row">
				...
				<view class='commodity-time' v-if="group.pintuan_start_status == 1">
                						<text>距结束仅剩</text>
                					...
                					<view class='commodity-time' v-if="group.pintuan_start_status == 2">
                						<text>即将开团</text>
                						<view class='commodity-day'>
                							<uni-countdown textColor="#fce250" :day="group.lasttime.day" :hour="group.lasttime.hour"
                							 :minute="group.lasttime.minute" :second="group.lasttime.second"></uni-countdown>
                						</view>
                					</view>
		...
		<view class="c-list">
			...
			<view class="eva-section">
				<view class="e-header" v-for="(item, index) in groupMemberList" :key="index">
					...
					<button type="primary" v-if="item.list.length>0 && item.status==1"
					class=" action-btn no-border add-cart-btn" @click="acceptGroup(item)">参与拼团</button>
		...
		if (id) {
			...
			if (data) {
                ...
                let timestamp = Date.parse(new Date()) / 1000;
                                                let timestamp1 = Date.parse(this.group.endTime) / 1000;
                    							let lasttime1 = timestamp1 - timestamp;
                    							console.log(lasttime1)
                    							this.lasttime = this.$common.timeToDateObj(lasttime1);
	...
	methods: {
		...
		async addGroup(item) {
			...
			if(this.sku.stock<1){
            		 ...
            		 let dataJson ={};
                                                        			dataJson.groupId=this.group.id;

                                                        			dataJson.id=item.id;
                                                        			dataJson.groupType=1;
                                                        			dataJson.type=1;
		...
		async acceptGroup(item) {
			...
			if(this.sku.stock<1){
            			...
            			let dataJson ={};
            			dataJson.groupId=item.groupId;
            			dataJson.mgid=item.id;
            			dataJson.id=this.goods.id;
            			dataJson.groupType=2;
            			dataJson.type=1;
            			dataJson.skuId=this.sku.id;
            			let url='/pages/order/createOrder?dataJson=' + JSON.stringify(dataJson)
